...
public class Message extends Object {
    ...
    public void removeSegment(Segment segment) {
        for(Iterator it=segments.iterator(); it.hasNext();) {
            Segment seg=(Segment)it.next();
            if(seg.getName().equals(segment.getName()) == true && (seg.getField(1).toString().equals(segment.getField(1).toString()) == true)) {
    ...
    public void write(java.io.Writer os) throws Exception {
        ...
        if (headerExists == true) {
            Segment head = generateHeader();
		if (head != null) {
			head.write(os,encoding);
            		os.write(13);
		}
        }
        for(Iterator it=segments.iterator();it.hasNext();) {
     ...
     */
    public Segment generateHeader() {
	Segment msh = getSegment("MSH");
	if (msh != null)
		return null;
