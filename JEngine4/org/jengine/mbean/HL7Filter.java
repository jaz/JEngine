...
public class HL7Filter implements Runnable, MessageListener {
	...
	public void setFilterProperties(String properties) {
		if(this.filter!=null) this.filter.init(ifName,properties);
	}

	public void setFilter(Filter filter) {
		this.filter = filter;
		if(filterProperties!=null) filter.init(ifName,filterProperties);
	}
	...
	public void start() {
		...
		try {
			...
			//and everything in between (filter) works.
	...
	public void onMessage(Message message) {
		...
		try
		{
			...
			/*
			 * HERE IS WHERE THE FILTERING WOULD TAKE PLACE
			 */
			String trx=inMsg.getText();
			if (filter != null ) trx=filter.execute(trx);
			else log.warn("No filtering performed");
